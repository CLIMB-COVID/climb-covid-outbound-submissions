...
if [ -f "submission.json" ]; then
    if [ ! -f "submit_accession.log" ]; then
        ...
        set +e
        submission_to_accession.py --response-mode json --response submission.json --csv $DATESTAMP.covv.csv --accessions-table $ARTIFACTS_ROOT/accessions/latest.accessions.tsv > submit_accession.log
        subret=$?
        set -e
