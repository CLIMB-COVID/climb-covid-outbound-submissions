...
if [ ! -f "$PHASE2_OK_FLAG" ]; then
    ...
    $NEXTFLOW_BIN run dehuman-post.nf -c $ELAN_SOFTWARE_DIR/elan.config --manifest $ELAN_DIR/staging/dh/$DATESTAMP/ascp.files.ls --datestamp $DATESTAMP --study PRJEB37886 --publish $ELAN_DIR $RESUME_FLAG > $PHASE2_LOG
    ret=$?
    if [ $ret -ne 0 ]; then
