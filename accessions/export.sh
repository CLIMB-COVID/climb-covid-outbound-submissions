...
ocarina --env --oauth get pag --test-name 'cog-uk-elan-minimal-qc' --pass --task-wait --task-wait-attempts 120 --odelimiter , \
    ...
    --ofield accession.ena-assembly.secondary_accession ena_assembly.secondary_accession '' > cog_uk_accessions.csv 2> err

$OUTBOUND_SOFTWARE_DIR/accessions/accessions_export_parser.py cog_uk_accessions.csv $ARTIFACTS_ROOT/accessions/$DATESTAMP.accessions.tsv 2> $EAGLEOWL_LOGS/accessions/$DATESTAMP.log
